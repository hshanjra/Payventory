    ...
    <View className={cn('w-full', className)}>
      <View className="relative">
        ...
        <TouchableOpacity onPress={() => setIsVisible(true)} disabled={isDisabled}>
          <View
            className={cn(
              'flex-row items-center justify-between rounded-xl border py-4 pl-3 pr-11',
              {
        ...
        <MaterialIcons
          ...
          size={24}
          className={cn('absolute right-4 top-1/2 -translate-y-[50%]', {
            'text-white': variant === 'secondary' && selectedOptions.length > 0,
          })}
          style={{
            color:
              variant === 'secondary' && selectedOptions.length > 0
                ? colors.primaryFg
                : colors.fgMuted,
          }}
        />
      ...
      {error && (
        <Text className={cn('mt-1 text-sm', errorClassName)} style={{ color: colors.error }}>
          {error.message}
        </Text>
      )}
