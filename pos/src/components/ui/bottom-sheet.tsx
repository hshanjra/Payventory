...
export interface BottomSheetProps extends Pick<ModalProps, 'visible' | 'onRequestClose'> {
  ...
  children:
    | React.ReactNode
    | ((props: { animateOut: (callback?: () => void) => void }) => React.ReactNode);
}
  ...
  return (
    <Modal
      ...
      <GestureHandlerRootView style={{ flex: 1 }}>
        <Animated.View
          className={cn('flex-1 items-center justify-end bg-black/50', className)}
          style={[
            {
          ...
          <Animated.View
            ...
            style={[
              {
                maxHeight:
                  windowDimensions.height - safeAreaInsets.bottom - safeAreaInsets.top - 16,
              },
              sheetStyle,
            ]}>
            <View
              className={cn(
                'w-full shrink grow-0 overflow-hidden rounded-t-2xl bg-white',
                containerClassName
              )}
              style={{
              ...
              {(title || showCloseButton) && (
                <View
                  className={cn(
                    'shrink-0 grow-0 flex-row items-center justify-between gap-2 p-4',
                    headerClassName
                  )}>
              ...
              <View className={cn('shrink grow-0 px-4', contentClassName)}>
